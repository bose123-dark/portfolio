...
populate_data.py     # Script to populate the database with initial sample data</code></pre>
    ...
    <h3>2. Clone and Navigate</h3>
    <p>Navigate to the project directory in your terminal:</p>
    <pre><code>cd "Gaming Website"</code></pre>

    <h3>3. Virtual Environment Setup</h3>
    <p>Create and activate a Python virtual environment to keep dependencies isolated:</p>
    <pre><code># Create a virtual environment
python -m venv venv
...
source venv/bin/activate</code></pre>
    ...
    <p>Install the required Python packages from the requirements manifest:</p>
    <pre><code>pip install -r requirements.txt</code></pre>

    <h3>5. Run Database Migrations</h3>
    <p>Apply database migrations to set up the default database schema:</p>
    <pre><code>python manage.py migrate</code></pre>

    <h3>6. Populate Sample Data (Optional)</h3>
    <p>Populate the database with initial sample data for games and blog posts:</p>
    <pre><code>python populate_data.py</code></pre>

    <h3>7. Create an Admin Account (Optional)</h3>
    <p>Create a superuser to access the Django admin panel:</p>
    <pre><code>python manage.py createsuperuser</code></pre>
    ...
    <h2>⚙️ Administration & Control Panel</h2>
    <p>You can access the Django admin panel by navigating to <a href="http://127.0.0.1:8000/admin/" target="_blank">http://127.0.0.1:8000/admin/</a> and logging in with your superuser credentials. From there, you can easily manage users, orders, and blog posts.</p>
</div>
`
};
